...
/**
 * 这个类实际上是任务分发类,和Scheduler和Timer都没关系；
 */
@Slf4j
@Service
public class SchedulerTimerService {
    ...
    public void doRun(SchedulerTimerTask task, Set<String> failoverList) {
        ...
        //当前任务分发的dispatchVersion是用时间戳来表示的，存在优化的空间;
        //todo
        ...
        try {
            ...
            OpenjobSpringContext
                    .getBean(SchedulerTimerService.class)
            ...
            // Add failover list.
            //当前failoverList徒有其名，并没有过滤失效worker的逻辑；
            //todo
            //或许可以加重试机制；
    ...
    public void updateByDispatcher(String workerAddress, Long jobId, Long instanceId, Long dispatchVersion, InstanceStatusEnum statusEnum, String message) {
        ...
        //这里的updateByRunning的条件判断仅仅在这个方法中进行，在DAO和repository中没有再次判断；
        //dispatchVersion应该是为了乐观锁设计的；
        //但是没有乐观锁的实现;
        //todo
        // Running to update.
        if (InstanceStatusEnum.RUNNING
                .getStatus()
    ...
    private ServerSubmitJobInstanceRequest getServerSubmitJobInstanceRequest(SchedulerTimerTask task, Long dispatchVersion) {
        ...
        submitReq.setExecuteOnce(Optional
                .ofNullable(task.getExecuteOnce())
                .orElse(CommonConstant.NO));
        return submitReq;
    }
}
