...
public class RedisUtil {
    ...
    public static List<Object> popAndRemoveFromList(String key, Integer count) {
        ...
        final List<Object> txResults = RedisUtil
                .getTemplate()
    ...
    public static <T, R> List<R> multiOrElseGet(String prefix, List<T> ids, Function<List<T>, Map<T, R>> function, Duration duration) {
        Pair<List<String>, List<T>> pairKeys = getKeyGenerator(KeyGenerator.<T>defGenerator(prefix), ids);
    ...
    public static <R> R orElseGet(String key, Supplier<R> supplier, Duration duration) {
        Optional
                .ofNullable(key)
                .orElseThrow(() -> new IllegalArgumentException("Key is empty."));
    ...
    private static <T, R> List<R> doOrElseGet(Pair<List<String>, List<T>> keyPairs, Function<List<T>, Map<T, R>> function, Duration duration) {
        ...
        //todo
        //MSET操作和设置过期时间的操作非原子，在时间窗口内可能看到未设置过期时间的值。
        //优化方向：1.逐个设置value带上过期时间，可以保证单个值设置的原子性。2.用lua脚本（但要考虑单个lua脚本过长的问题）。
        // Query from store.
        ...
        if (result != null && !result.isEmpty()) {
            ...
            redisTemplate.executePipelined((RedisCallback<Void>) connection -> {
                store.forEach((k, v) -> {
                    ...
                    Objects
                            .requireNonNull(keyForIndex.get(k))
                            .forEach(index -> value.set(index, v));
                });
                return null;
            });
        }
    ...
    /**
     * Remove null item.
     * 这一步是redis和db都为空的情况下，就要将值从本地缓存中删除；
     *
     * @param value value
     * @param <R>   type
     * @return List
