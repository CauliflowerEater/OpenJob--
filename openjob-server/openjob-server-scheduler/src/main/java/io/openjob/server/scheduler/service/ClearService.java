...
public class ClearService {
    ...
    /**
     * Clear data
     * 槽位1的节点是cluster的manager节点，负责执行清理任务;
     * 其他的节点在执行clear方法时就会返回，确保清除操作只会执行一次;
     */
    ...
    /**
     * Do clear
     * 清除结束的 job instance, delay instance;
     * 清除失败的 server, worker;
     *
    ...
    private void doClear() throws Exception {
        ...
        Integer jobKeepDays = ClusterContext
                .getSystem()
                .getJobKeepDays();
        Integer delayKeepDays = ClusterContext
                .getSystem()
                .getDelayKeepDays();
        Integer serverKeepDays = ClusterContext
                .getSystem()
                .getServerKeepDays();
        Integer workerKeepDays = ClusterContext
                .getSystem()
                .getWorkerKeepDays();
