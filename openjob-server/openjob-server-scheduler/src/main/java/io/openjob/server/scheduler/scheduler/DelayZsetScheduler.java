    ...
    static class ZsetRunnable extends AbstractZsetRunnable {
        ...
        protected void push2FailZset(RedisOperations<String, Object> operations, String originZsetKey, String topic, List<DelayInstanceAddRequestDTO> list) {
            ...
            list.forEach(d -> {
                ...
                operations
                        .opsForList()
                        .remove(currentTopicKey, 0, d.getTaskId());

                // Add to fail zset
                operations
                        .opsForZSet()
                        .add(zsetKey, taskId, timestamp);
            });
            ...
            operations
                    .opsForZSet()
                    .remove(originZsetKey, taskIds.toArray());
        ...
        protected void ignoreTaskList(RedisOperations<String, Object> operations, String zsetKey, List<DelayInstanceAddRequestDTO> list) {
            ...
            operations
                    .opsForZSet()
                    .remove(zsetKey, taskIds.toArray());
            ...
            String statusListKey = CacheUtil.getStatusListKey(DelaySlotUtil.getStatusListSlotId(UUID
                    .randomUUID()
                    .toString()));
            Stream<DelayInstanceStatusRequestDTO> statusList = list
                    .stream()
                    .map(d -> {
        ...
        private void appendProcessorLog(List<String> taskIds) {
            try {
                List<ProcessorLogDTO> processorLogs = taskIds
                        .stream()
                        .map(tid -> {
