...
public class DelayInstanceScheduler {
    ...
    public DelayTopicPullResponseDTO pullTopicList(DelayTopicPullRequestDTO pullRequestDTO) {
        ...
        //从DB中根据appid查询所有Delay；
        //将查询结果转化为DelayTopicPullDTO；
        //返回DelayTopicPullDTO的list；
        List<DelayTopicPullDTO> pullTopics = this.delayData
                .getListByAppId(app.getId())
                .stream()
    ...
    public List<DelayInstancePullResponseDTO> pullByTopic(String workerAddress, DelayItemPullRequestDTO pullRequestDTO) {
        ...
        List<String> taskIds = delayList
                .stream()
                .map(String::valueOf)
                .collect(Collectors.toList());

        //给taskId加上worker address的键前缀，作为redis中worker address的key；
        //用于根据taskId 查询 worker address；
        // Set delay instance worker address.
        Map<String, String> taskIdMap = taskIds
                .stream()
                .collect(Collectors.toMap(CacheUtil::getDelayDetailWorkerAddressKey, i -> workerAddress));
        RedisUtil
                .getTemplate()
                .opsForValue()
                .multiSet(taskIdMap);

        //这个pid是用来判断当前的任务来自主队列还是失败队列；
        return this.delayData
                .getDelayInstanceList(taskIds)
                .stream()
                .map(di -> {
    ...
    public void report(List<DelayInstanceStatusRequestDTO> statusList) {
        ...
        Map<String, List<DelayInstanceStatusRequestDTO>> zsetKeyMap = statusList
                .stream()
                .collect(Collectors.groupingBy(DelayInstanceStatusRequestDTO::getZsetKey));

        List<DelayInstanceStatusRequestDTO> completeList = statusList
                .stream()
                .filter(d -> TaskStatusEnum.isDelayComplete(d.getStatus()))
                .collect(Collectors.toList());

        // Detail cache keys.
        List<String> completeDetailKeys = completeList
                .stream()
                .map(d -> CacheUtil.getDelayDetailTaskIdKey(d.getTaskId()))
                .collect(Collectors.toList());

        // Worker address keys.
        List<String> completeAddressKeys = completeList
                .stream()
                .map(d -> CacheUtil.getDelayDetailWorkerAddressKey(d.getTaskId()))
                .collect(Collectors.toList());

        // retry times keys.
        List<String> completeRetryKeys = completeList
                .stream()
                .map(d -> CacheUtil.getDelayRetryTimesKey(d.getTaskId()))
                .collect(Collectors.toList());

        // Delay status list key.
        //取随机数将DelayInstanceStatusRequestDTO存入随机的status slot中，statusDTO由statusListScheduler调度；
        String statusListKey = CacheUtil.getStatusListKey(DelaySlotUtil.getStatusListSlotId(UUID
                .randomUUID()
                .toString()));
        ...
        RedisUtil
                .getTemplate()
                .executePipelined(new SessionCallback<List<Object>>() {
                    @Override
                    public List<Object> execute(@Nonnull RedisOperations operations) throws DataAccessException {
                        ...
                        zsetKeyMap.forEach((k, list) -> {
                            List<String> completeStatusList = list
                                    .stream()
                                    .filter(d -> TaskStatusEnum.isDelayComplete(d.getStatus()))
                            ...
                            if (!CollectionUtils.isEmpty(completeStatusList)) {
                                operations
                                        .opsForZSet()
                                        .remove(k, completeStatusList.toArray());
                            }
                        });
                        ...
                        operations
                                .opsForList()
                                .rightPushAll(statusListKey, statusList.toArray());
                        operations.exec();
                        return null;
                    }
                });
    ...
    public DelayInstanceStopResponseDTO stop(DelayInstanceStopRequestDTO stopRequest) {
        ...
        Object workerAddress = RedisUtil
                .getTemplate()
                .opsForValue()
                .get(addressKey);
        ...
        RedisUtil
                ...
                .executePipelined(new SessionCallback<List<Object>>() {
                    ...
                    public List<Object> execute(@Nonnull RedisOperations operations) throws DataAccessException {
                        ...
                        operations
                                .opsForValue()
                                .set(detailKey, addRequest);
                        operations
                                .opsForZSet()
                                .add(zsetKey, taskId, addRequest.getExecuteTime());
                        operations
                                .opsForList()
                                .rightPush(listKey, taskId);
                        operations.exec();
                        return null;
                    }
                });
        ...
        RedisUtil
                ...
                .executePipelined(new SessionCallback<List<Object>>() {
                    ...
                    public List<Object> execute(@Nonnull RedisOperations operations) throws DataAccessException {
                        ...
                        operations
                                .opsForZSet()
                                .remove(zsetKey, taskId);
                        operations
                                .opsForList()
                                .remove(listKey, 1, taskId);
                        operations.exec();
                        return null;
                    }
                });
