...
public class JobSchedulingService {
    ...
    public void scheduleFailoverJob() {
        ...
        if (!CollectionUtils.isEmpty(unDispatchList)) {
            ...
            log.info("Retry dispatch list!{}", unDispatchList
                    .stream()
        ...
        if (!CollectionUtils.isEmpty(failoverList)) {
            ...
            log.info("Retry failover list!{}", failoverList
                    .stream()
                    .map(JobInstance::getId)
                    .collect(Collectors.toList()));
        }
        ...
        dispatchList.forEach(js -> {
            ...
            //如果状态为RUNNING且task master存在，则不进行重试;
            //这代表当前的instance仍在worker上正常运行，如果重新调度，可能会导致任务重复执行;
    ...
    public void scheduleCronJob(List<Long> currentSlots) {
        ...
        //因为当前的execute time下的任务instance已经创建，下一个逻辑块是要计算下一次的execute time；
        jobs.forEach(j -> {
            if (TimeExpressionTypeEnum.isCron(j.getTimeExpressionType())) {
                ...
                //这个逻辑块调度的是fixed rate的任务，如果首次的执行时间在time stamp之前，那这个任务插入timing wheel就会立刻执行：
                //实际的执行时间就会是timestamp；
                //但如果下次的执行时间（当前已创立intance的执行时间）在timestamp之后，那这个任务就会插入timing wheel相应的bucket中；
                //那下次的执行时间就应该是计划的执行时间+时间间隔;
                // Fixed rate
    ...
    /**
     * Create job instance.
     * 根据输入的job list创建job instance list，并添加到时间轮中；
     *
     * @param jobs jobs
     */
    ...
    //如果JobInstance直接转化为TimerTask,那JobInstanceTaskId是干什么的？

    /**
     * 将job instance转化为scheduler timer task；
     */
    ...
    /**
     * 如果job是cron类型，则计算下次执行时间；
     * 如果job是fixed rate类型，则计算下次执行时间；
     * 因为job的nextExecuteTime属性已经是下次的执行时间，这个方法计算的是下下次的执行时间；
     */
    private Long calculateNextExecuteTime(Job job, Long now) throws ParseException {
        ...
        if (TimeExpressionTypeEnum.CRON_TYPES.contains(job.getTimeExpressionType())) {
            ...
            //在job初始化的时候就会根据cron表达式计算出next execute time;
            //初始化next execute time调用的是setNextExecuteTime方法，会调用parseTimeExpression方法;
            //parseTimeExpression(String timeExpression, Date afterDate);
            //setNextexecuteTime为parseTimeExpression方法默认传afterDate为null;
            //paeseTimeExpression在afterDate为null时会获取当前时间作为代替，由此确定了最初的next execute time;
            ...
            //afterTime 已经是下一次执行的时间了;
            //getNextValidTimeAfter计算的事实上是下下次的执行时间;
    ...
    /**
     * 计算job的下次执行时间；
     * 如果job的执行时间在下一个时间窗口之内，则创建job的实例；
     * 更新DB中的nextExecuteTime；
     *
     * @param j
    ...
    private void calculateCronTimeExpression(Job j) throws ParseException {
        ...
        //因为调用链的上游已经将nextExecuteTime创建为了Instance，因此可以被下下次的execute time覆盖;
    ...
    /**
     * 转化为失败重试的SchedulerTimerTask;
     * 因为是重试，所以需要设置circle id；
     *
     * @param js
     * @return
     */
