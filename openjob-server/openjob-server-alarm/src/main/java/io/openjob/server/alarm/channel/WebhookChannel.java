...
public class WebhookChannel extends AbstractChannel {
    ...
    private void doSend(String url, AlarmDTO alarmDTO) throws IOException, NoSuchAlgorithmException, InvalidKeyException {
        ...
        webhookRequest.setAppName(alarmDTO
                .getApp()
                .getName());
        webhookRequest.setProcessor(alarmDTO
                .getEvent()
                .getName());
        webhookRequest.setMessage(Optional
                .ofNullable(alarmDTO
                        .getEvent()
                        .getMessage())
                .orElse(""));
        webhookRequest.setTimestamp(timestamp);
        webhookRequest.setJobId(alarmDTO
                .getEvent()
                .getJobUniqueId());
        webhookRequest.setJobInstanceId(alarmDTO
                .getEvent()
                .getInstanceId());
        webhookRequest.setEvent(alarmDTO
                .getEvent()
                .getName());
        webhookRequest.setSign(this.getFeishuOrWebhookSign(alarmDTO
                .getAlertRule()
                .getSecret(), timestamp));

        // Job or delay
        if (AlarmEventEnum.isJobEvent(alarmDTO
                .getEvent()
                .getName())) {
            webhookRequest.setJobName(alarmDTO
                    .getJob()
                    .getName());
            webhookRequest.setProcessor(alarmDTO
                    .getJob()
                    .getProcessorInfo());
        } else if (AlarmEventEnum.isDelayEvent(alarmDTO
                .getEvent()
                .getName())) {
            webhookRequest.setJobName(alarmDTO
                    .getDelay()
            ...
            throw new RuntimeException("Event not supported! event=" + alarmDTO
                    .getEvent()
                    .getName());
        }
