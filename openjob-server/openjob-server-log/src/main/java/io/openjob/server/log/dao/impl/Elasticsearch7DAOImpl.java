...
public class Elasticsearch7DAOImpl implements LogDAO {
    ...
    public void batchAdd(List<ProcessorLogDTO> jobInstanceTaskLogs) {
        ...
        this.elasticsearch7Client
                .getClient()
                .bulkAsync(bulkRequest, this.elasticsearch7Client.getRequestOptions(), new BulkListener());
    }
    ...
    public List<ProcessorLogDTO> queryByScroll(String taskUniqueId, Long time, Integer size) throws Exception {
        ...
        //todo
        //隐患在于taskId.keyword,这个字段是否真的是预期中的字段；
        // `taskId=?`
        ...
        return this
                .queryResult(searchRequest, 0, size)
                .getList();
    }
    ...
    private PageDTO<ProcessorLogDTO> queryResult(SearchRequest searchRequest, Integer page, Integer size) throws IOException {
        SearchResponse searchResponse = this.elasticsearch7Client
                .getClient()
                .search(searchRequest, this.elasticsearch7Client.getRequestOptions());
        SearchHit[] searchHit = searchResponse
                .getHits()
                .getHits();

        // Processor log list
        List<ProcessorLogDTO> processorLogList = Arrays
                .stream(searchHit)
                .map(h -> {
                    ...
                    fieldsMap.put(LogFieldConstant.MESSAGE, Optional
                            .ofNullable(processorLogElasticDTO.getMessage())
                            .orElse(""));
                    fieldsMap.put(LogFieldConstant.THROWABLE, Optional
                            .ofNullable(processorLogElasticDTO.getThrowable())
                            .orElse(""));
        ...
        pageDTO.setTotal(searchResponse
                .getHits()
                .getTotalHits().value);
        return pageDTO;
    }
    ...
    public void deleteByLastTime(Long lastTime) throws Exception {
        ...
        // Not exist index
        boolean exists = this.elasticsearch7Client
                .getClient()
                .indices()
                .exists(request, requestOptions);
        if (!exists) {
        ...
        // Get index
        GetIndexResponse getIndexResponse = this.elasticsearch7Client
                .getClient()
                .indices()
        ...
        Arrays
                ...
                .forEach(i -> {
                    ...
                    if (indexDate < lastDate) {
                        ...
                        try {
                            AcknowledgedResponse deleteResponse = this.elasticsearch7Client
                                    .getClient()
                                    .indices()
                                    .delete(deleteIndexRequest, requestOptions);
    ...
    public synchronized void doCreateIndex(String indexName) {
        try {
            ...
            boolean exists = this.elasticsearch7Client
                    .getClient()
                    .indices()
                    .exists(getIndexRequest, requestOptions);
            if (!exists) {
                CreateIndexRequest createIndexRequest = new CreateIndexRequest(indexName);
                CreateIndexResponse createIndexResponse = this.elasticsearch7Client
                        .getClient()
                        .indices()
